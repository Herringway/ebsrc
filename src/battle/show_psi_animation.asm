
SHOW_PSI_ANIMATION:
	BEGIN_C_FUNCTION_FAR
	STACK_RESERVE_VARS
	STACK_RESERVE_INT32
	STACK_RESERVE_INT32
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT16
	STACK_RESERVE_INT32
	STACK_RESERVE_INT32
	STACK_RESERVE_INT32
	STACK_RESERVE_PARAM_INT16
	END_STACK_VARS
	STA @VIRTUAL02
	LDA LOADED_BG_DATA_LAYER1 + loaded_bg_data::bitdepth
	AND #$00FF
	CMP #2
	BNE @UNKNOWN1
	LOADPTR UNKNOWN_7F8000, @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL07
	LOADPTR PSI_ANIM_GFX_SET_1 & $FF0000, @VIRTUAL06
	LDA @VIRTUAL02
	OPTIMIZED_MULT @VIRTUAL04, 12
	TAX
	LDA f:PSI_ANIM_CFG,X
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	STA @LOCAL00
	LDA @VIRTUAL06+2
	STA @LOCAL00+2
	MOVE_INT @LOCAL07, @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL01
	JSL DECOMP
	COPY_TO_VRAM3P @VIRTUAL06, $0000, $1000, 0
	.A16
	LDA #.LOWORD(CUR_TEXT_PAL) + BPP4PALETTE_SIZE * 3
	STA UNKNOWN_7E1BCA
	JMP @UNKNOWN6
@UNKNOWN1:
	LOADPTR UNKNOWN_7F0000, @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL07
	LOADPTR PSI_ANIM_GFX_SET_1 & $FF0000, @VIRTUAL06
	LDA @VIRTUAL02
	OPTIMIZED_MULT @VIRTUAL04, 12
	TAX
	LDA f:PSI_ANIM_CFG,X
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	STA @LOCAL00
	LDA @VIRTUAL06+2
	STA @LOCAL00+2
	MOVE_INT @LOCAL07, @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL01
	JSL DECOMP
	LOADPTR UNKNOWN_7F8000, @VIRTUAL0A
	LDX #0
	JMP @UNKNOWN4
@UNKNOWN2:
	LDA [@VIRTUAL06]
	STA [@VIRTUAL0A]
	INC @VIRTUAL06
	INC @VIRTUAL06
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	LDA [@VIRTUAL06]
	STA [@VIRTUAL0A]
	INC @VIRTUAL06
	INC @VIRTUAL06
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	LDA [@VIRTUAL06]
	STA [@VIRTUAL0A]
	INC @VIRTUAL06
	INC @VIRTUAL06
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	LDA [@VIRTUAL06]
	STA [@VIRTUAL0A]
	INC @VIRTUAL06
	INC @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL07
	MOVE_INT @VIRTUAL0A, @VIRTUAL06
	INC @VIRTUAL06
	INC @VIRTUAL06
	LDA [@LOCAL07]
	STA [@VIRTUAL06]
	MOVE_INT @LOCAL07, @VIRTUAL0A
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	INC @VIRTUAL06
	INC @VIRTUAL06
	LDA [@VIRTUAL0A]
	STA [@VIRTUAL06]
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	INC @VIRTUAL06
	INC @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL06
	LDA [@VIRTUAL0A]
	STA [@VIRTUAL06]
	MOVE_INT @VIRTUAL0A, @VIRTUAL06
	INC @VIRTUAL06
	INC @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL05
	MOVE_INT @LOCAL06, @VIRTUAL06
	MOVE_INT @VIRTUAL06, @VIRTUAL0A
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	LDA [@LOCAL05]
	STA [@VIRTUAL0A]
	MOVE_INT @LOCAL05, @VIRTUAL06
	INC @VIRTUAL06
	INC @VIRTUAL06
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	LDA #0
	STA [@VIRTUAL0A]
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	STA [@VIRTUAL0A]
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	STA [@VIRTUAL0A]
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	STA [@VIRTUAL0A]
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	STA [@VIRTUAL0A]
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	STA [@VIRTUAL0A]
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	STA [@VIRTUAL0A]
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	STA [@VIRTUAL0A]
	INC @VIRTUAL0A
	INC @VIRTUAL0A
	INX
@UNKNOWN4:
	CPX #256
	BCCL @UNKNOWN2
	COPY_TO_VRAM3 UNKNOWN_7F8000, $0000, $2000, 0
	.A16
	LDA #.LOWORD(CUR_TEXT_PAL) + BPP4PALETTE_SIZE * 4
	STA UNKNOWN_7E1BCA
@UNKNOWN6:
	JSL WAIT_UNTIL_NEXT_FRAME
	LOADPTR PSI_ANIM_PALETTES, @VIRTUAL06
	LDA @VIRTUAL02
	ASL
	ASL
	ASL
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	STA @LOCAL00
	LDA @VIRTUAL06+2
	STA @LOCAL00+2
	LDX #8
	LDA #.LOWORD(UNKNOWN_7E1BAA)
	JSL MEMCPY16
	MOVE_INT @VIRTUAL06, @LOCAL00
	LDX #8
	LDA UNKNOWN_7E1BCA
	JSL MEMCPY16
	LOADPTR UNKNOWN_7F0000, @VIRTUAL0A
	MOVE_INT @VIRTUAL0A, @VIRTUAL06
	MOVE_INT @VIRTUAL06, UNKNOWN_7E1BA1
	SEP #PROC_FLAGS::ACCUM8
	LDA #1
	STA UNKNOWN_7E1B9E
	REP #PROC_FLAGS::ACCUM8
	LOADPTR PSI_ANIM_CFG, @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL07
	LDA @VIRTUAL02
	OPTIMIZED_MULT @VIRTUAL04, 12
	STA @LOCAL04
	INC
	INC
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	SEP #PROC_FLAGS::ACCUM8
	LDA [@VIRTUAL06]
	STA UNKNOWN_7E1B9E+1
	REP #PROC_FLAGS::ACCUM8
	LDA @LOCAL04
	CLC
	ADC #6
	MOVE_INTX @LOCAL07, @VIRTUAL06
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	SEP #PROC_FLAGS::ACCUM8
	LDA [@VIRTUAL06]
	STA UNKNOWN_7E1BA0
	REP #PROC_FLAGS::ACCUM8
	LDA @LOCAL04
	INC
	INC
	INC
	MOVE_INTX @LOCAL07, @VIRTUAL06
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	SEP #PROC_FLAGS::ACCUM8
	LDA [@VIRTUAL06]
	STA UNKNOWN_7E1BA8
	REP #PROC_FLAGS::ACCUM8
	LDA @LOCAL04
	INC
	INC
	INC
	INC
	MOVE_INTX @LOCAL07, @VIRTUAL06
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	SEP #PROC_FLAGS::ACCUM8
	LDA [@VIRTUAL06]
	STA UNKNOWN_7E1BA5
	REP #PROC_FLAGS::ACCUM8
	LDA @LOCAL04
	CLC
	ADC #5
	MOVE_INTX @LOCAL07, @VIRTUAL06
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	SEP #PROC_FLAGS::ACCUM8
	LDA [@VIRTUAL06]
	STA UNKNOWN_7E1BA6
	STZ UNKNOWN_7E1BA7
	LDA #1
	STA UNKNOWN_7E1BA9
	REP #PROC_FLAGS::ACCUM8
	LDA @LOCAL04
	CLC
	ADC #8
	MOVE_INTX @LOCAL07, @VIRTUAL06
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	LDA [@VIRTUAL06]
	AND #$00FF
	STA UNKNOWN_7E1BCC
	LDA @LOCAL04
	CLC
	ADC #9
	MOVE_INTX @LOCAL07, @VIRTUAL06
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	LDA [@VIRTUAL06]
	AND #$00FF
	STA UNKNOWN_7E1BCE
	LDA @LOCAL04
	CLC
	ADC #10
	MOVE_INTX @LOCAL07, @VIRTUAL06
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	LDA [@VIRTUAL06]
	AND #$001F
	STA UNKNOWN_7E1BD0
	LDA [@VIRTUAL06]
	LSR
	LSR
	LSR
	LSR
	LSR
	AND #$001F
	STA UNKNOWN_7E1BD2
	SEP #PROC_FLAGS::INDEX8
	LDY #10
	LDA [@VIRTUAL06]
	JSL ASR8_UNKNOWN1
	AND #$001F
	STA UNKNOWN_7E1BD4
	LOADPTR PSI_ANIM_POINTERS, @VIRTUAL06
	LDA @VIRTUAL02
	ASL
	ASL
	CLC
	ADC @VIRTUAL06
	STA @VIRTUAL06
	REP #PROC_FLAGS::INDEX8
	DEREFERENCE_PTR_TO @VIRTUAL06, @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL00
	MOVE_INT @VIRTUAL0A, @VIRTUAL06
	MOVE_INT @VIRTUAL06, @LOCAL01
	JSL DECOMP
	JSL UNKNOWN_C2DE0F
	PROMOTENEARPTR CUR_SPRITE_PALS, @VIRTUAL06
	REP #PROC_FLAGS::ACCUM8
	MOVE_INT @VIRTUAL06, @LOCAL00
	LDX #128
	LDA #.LOWORD(CUR_SPRITE_PALS) + BPP4PALETTE_SIZE * 4
	JSL MEMCPY16
	LDA #0
	STA @LOCAL04
	BRA @UNKNOWN8
@UNKNOWN7:
	ASL
	TAX
	STZ UNKNOWN_7EAEE7,X
	LDA @LOCAL04
	INC
	STA @LOCAL04
@UNKNOWN8:
	CMP #4
	BCC @UNKNOWN7
	LDX CURRENT_TARGET
	LDA a:battler::consciousness,X
	AND #$00FF
	BEQL @UNKNOWN26
	LDX CURRENT_TARGET
	LDA a:battler::ally_or_enemy,X
	AND #$00FF
	CMP #1
	BNEL @UNKNOWN26
	STZ UNKNOWN_7EAD9A
	LDA @VIRTUAL02
	OPTIMIZED_MULT @VIRTUAL04, 12
	CLC
	ADC #7
	TAX
	LDA f:PSI_ANIM_CFG,X
	AND #$00FF
	BEQ @UNKNOWN12
	CMP #3
	BEQ @UNKNOWN12
	CMP #1
	BEQ @UNKNOWN14
	CMP #2
	BEQL @UNKNOWN20
	JMP @UNKNOWN24
@UNKNOWN12:
	LDX CURRENT_TARGET
	LDA a:battler::sprite_x,X
	AND #$00FF
	STA @VIRTUAL02
	LDA #128
	SEC
	SBC @VIRTUAL02
	STA UNKNOWN_7EAD9A
	LDX CURRENT_TARGET
	LDA a:battler::sprite_y,X
	AND #$00FF
	STA @VIRTUAL02
	LDA #144
	SEC
	SBC @VIRTUAL02
	STA UNKNOWN_7EAD9C
	LDX CURRENT_TARGET
	LDA a:battler::sprite,X
	JSR GET_BATTLE_SPRITE_HEIGHT
	CMP #8
	BNE @UNKNOWN13
	LDA UNKNOWN_7EAD9C
	CLC
	ADC #16
	STA UNKNOWN_7EAD9C
@UNKNOWN13:
	SEP #PROC_FLAGS::ACCUM8
	LDA #1
	LDX CURRENT_TARGET
	STA a:battler::use_alt_spritemap,X
	LDX CURRENT_TARGET
	REP #PROC_FLAGS::ACCUM8
	LDA a:battler::vram_sprite_index,X
	AND #$00FF
	ASL
	TAX
	LDA #1
	STA UNKNOWN_7EAEE7,X
	JMP @UNKNOWN24
@UNKNOWN14:
	LDX CURRENT_TARGET
	LDA a:battler::sprite_y,X
	AND #$00FF
	STA @VIRTUAL02
	LDA #144
	SEC
	SBC @VIRTUAL02
	STA UNKNOWN_7EAD9C
	LDY #0
	STY @LOCAL04
	LDA #.LOWORD(BATTLERS_TABLE) + (.SIZEOF(battler) * 8)
	STA @VIRTUAL02
	LDX #8
	STX @LOCAL03
	BRA @UNKNOWN18
@UNKNOWN15:
	LDX @VIRTUAL02
	LDA a:battler::consciousness,X
	AND #$00FF
	BEQ @UNKNOWN17
	LDX @VIRTUAL02
	LDA a:battler::ally_or_enemy,X
	AND #$00FF
	CMP #1
	BNE @UNKNOWN17
	LDX @VIRTUAL02
	LDA a:battler::afflictions + STATUS_GROUP::PERSISTENT_EASYHEAL,X
	AND #$00FF
	CMP #1
	BEQ @UNKNOWN17
	LDX @VIRTUAL02
	SEP #PROC_FLAGS::ACCUM8
	LDA a:battler::sprite_y,X
	LDX CURRENT_TARGET
	CMP a:battler::sprite_y,X
	BNE @UNKNOWN17
	LDX @VIRTUAL02
	REP #PROC_FLAGS::ACCUM8
	LDA a:battler::sprite,X
	JSR GET_BATTLE_SPRITE_HEIGHT
	CMP #8
	BNE @UNKNOWN16
	LDY #1
	STY @LOCAL04
@UNKNOWN16:
	SEP #PROC_FLAGS::ACCUM8
	LDA #1
	LDX @VIRTUAL02
	STA a:battler::use_alt_spritemap,X
	LDX @VIRTUAL02
	REP #PROC_FLAGS::ACCUM8
	LDA a:battler::vram_sprite_index,X
	AND #$00FF
	ASL
	TAX
	LDA #1
	STA UNKNOWN_7EAEE7,X
@UNKNOWN17:
	REP #PROC_FLAGS::ACCUM8
	LDA @VIRTUAL02
	CLC
	ADC #.SIZEOF(battler)
	STA @VIRTUAL02
	LDX @LOCAL03
	INX
	STX @LOCAL03
@UNKNOWN18:
	CPX #32
	BCCL @UNKNOWN15
	LDY @LOCAL04
	BEQ @UNKNOWN24
	LDA UNKNOWN_7EAD9C
	CLC
	ADC #16
	STA UNKNOWN_7EAD9C
	BRA @UNKNOWN24
@UNKNOWN20:
	LDA #16
	STA UNKNOWN_7EAD9C
	LDY #.LOWORD(BATTLERS_TABLE) + .SIZEOF(battler) * 8
	LDX #8
	STX @LOCAL02
	BRA @UNKNOWN23
@UNKNOWN21:
	LDA a:battler::consciousness,Y
	AND #$00FF
	BEQ @UNKNOWN22
	LDA a:battler::ally_or_enemy,Y
	AND #$00FF
	CMP #1
	BNE @UNKNOWN22
	LDA a:battler::afflictions + STATUS_GROUP::PERSISTENT_EASYHEAL,Y
	AND #$00FF
	CMP #1
	BEQ @UNKNOWN22
	SEP #PROC_FLAGS::ACCUM8
	LDA #1
	STA a:battler::use_alt_spritemap,Y
	REP #PROC_FLAGS::ACCUM8
	LDA a:battler::vram_sprite_index,Y
	AND #$00FF
	ASL
	TAX
	LDA #1
	STA UNKNOWN_7EAEE7,X
@UNKNOWN22:
	TYA
	CLC
	ADC #.SIZEOF(battler)
	TAY
	LDX @LOCAL02
	INX
	STX @LOCAL02
@UNKNOWN23:
	CPX #32
	BCC @UNKNOWN21
@UNKNOWN24:
	LDA LOADED_BG_DATA_LAYER1 + loaded_bg_data::bitdepth
	AND #$00FF
	CMP #2
	BNE @UNKNOWN25
	LDA UNKNOWN_7EAD9A
	STA BG2_X_POS
	LDA UNKNOWN_7EAD9C
	STA BG2_Y_POS
	BRA @UNKNOWN26
@UNKNOWN25:
	LDA UNKNOWN_7EAD9A
	STA BG1_X_POS
	LDA UNKNOWN_7EAD9C
	STA BG1_Y_POS
@UNKNOWN26:
	END_C_FUNCTION
