...
GENERATE_BATTLEBG_FRAME: ;$C2C92D
	...
	AND #$00FF
	BEQ @UNKNOWN0
	JMP a:.LOWORD(@TARGET_BG_LAYER_SELECTION_COMPLETE)
@UNKNOWN0:
	LDA $1B
	CLC
...
@UNKNOWN2:
	...
	CMP #$0001
	BNE @UNKNOWN3
	JMP a:.LOWORD(@PALETTE_SHIFTING_STYLE1)
@UNKNOWN3:
	CMP #$0003
	BNE @UNKNOWN4
	JMP a:.LOWORD(@PALETTE_SHIFTING_STYLE3)
@UNKNOWN4:
	JMP a:.LOWORD(@PALETTE_SHIFTING_DONE)
...
@TARGET_BG_LAYER_SELECTION_COMPLETE:
	...
	LDA a:.LOWORD(RAM),X
	BNE @UNKNOWN35
	JMP a:.LOWORD(@DISTORTION_DMA_DONE)
@UNKNOWN35:
	DEC
	STA a:.LOWORD(RAM),X
	BEQ @UNKNOWN36
